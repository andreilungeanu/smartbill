...
`require` is unchanged, so the installed package is identical to 2.1.0.

### Changed
- CI tests Laravel 13 only. Laravel 11 and 12 cannot install the Pest 5 toolchain, and
  Laravel 11's security window closed in March 2026. Both remain installable and
  supported; they are no longer exercised by the suite
- The Pest 4 fallback used for the Laravel 12 row is gone, along with the testbench floor
  it needed
