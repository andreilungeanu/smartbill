...
- Target **PHP 8.2** in `src/` — avoid 8.3+ syntax. Tests are Pest 5 and run on PHP 8.4+; CI runs the same suite on Pest 4 for the Laravel 12 row, so keep tests compatible with both.
- Every endpoint method must follow the pattern documented in [CLAUDE.md](../CLAUDE.md): route the response through `decode()` or `download()` from [BaseEndpoint](../src/Endpoints/BaseEndpoint.php), and send query-string PUT/DELETE via `sendQuery()`.
